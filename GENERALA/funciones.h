...
void cargarVector (int v[], int tam, int fila); /// Declaraci�n funci�n carga vector enteros.
void mostrarVector (int v[], int tam);          /// Declaraci�n funci�n muestra vector enteros.
void cargarNombre1 (char v[], int tam);         /// Declaraci�n funci�n carga vector nombres.
void copiarNombre1 (char v[], char v2[]);        /// Declaraci�n funci�n muestra vector nombre.
...
void mostrarTabla (int v[10][2], int tam, char vecNom1[20], char vecNom2[20]);      /// Muestra los puntos de cada posici�n del vector.
...
cout<<" Nota: Siempre que se de este resultado, se tomar� el valor mas alto disponible. Si obtuve 3 dados "<<endl;
...
void cargarNombre1 (char *v, int tam){
    ...
    fflush(stdin);                          ///Evita que salte la l�nea.
    cin.getline (v,19);
...
int i, j, vecDados[5], puntos[10][2],cantidadJug=1, jugador=0, rondas, totalPuntaje;
    ...
    for(i=1;i<=rondas;i++){
        ...
        system("pause");
        system("cls");
        mostrarTabla(puntos,cantidadJug,nombre1, nombre2);                  /// Env�a a funci�n tabla el vector de puntos, la cantidad de jugadores
                                                                            /// y el nombre.
        jugador=0;
        tirada(vecDados,5,puntos,jugador,cantidadJug,nombre1, nombre2,&i, &j, totalPuntaje, lanzamiento1, lanzamiento2);     /// Le env�a a funci�n tirada el vector de dados, el tama�o de vector dados,
    ...
    while (contador<3&&(decision=='s'||decision=='S')){
        ...
        if(jugador==0||jugador==2){
            ...
            cout<<"--------------------------------------------------------------------"<<endl<<endl;
                ...
                if(jugador<2&&contador==1){                                 /// 2 siempre aleatorio / 4 opci�n modo de tirada.
                    gotoxy(100,6);
                ...
                cin>>cantDados;
                    if (cantDados<5){
                        ...
                        if(jugador>1){
                            ...
                            gotoxy(76,21+(salto));                                 /// Seg�n los dados que elija el usuario se carga aleatoreamente esa posici�n (n-1).
                            cout<<"Dado numero: ";
                            cin>>n;
                            gotoxy(76,22+(salto));
                        ...
                        for(i=0;i<cantDados;i++){                   /// Seg�n los dados que elija el usuario se carga aleatoreamente esa posici�n (n-1).
        ...
        bool corte=false;
            for(jug=jugador;jug<2;jug++){
                while(corte==false){                                      /// Evita que ingrese a chequear jugador 2 cuando solamente tiene que chequear jugador 1.
                    if (puntos[j][jug]==-1&&validacion[j]>0){             /// Entra si puntaje en tabla est� en -1 y la validacion tiene puntaje (mayor a 0).
                        opcion++;
                        contador2=contador2+1;
                        gotoxy(70,22+opcion);
                        cout<<opcion<<" - "<<nombresJuegos[j][0]<<" = "<<validacion[j]<<endl; /// Muestra opci�n, nombre de juego disponible y valor de combinaci�n.
...
/// Switch selecci�n anulaci�n combinaci�n.
    switch(opc){
...
/// Suma los n�meros iguales dentro del vector.
